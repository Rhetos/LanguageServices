...
Reference a.b.x p ";
        ...
        public void CorrectKeywords(int line, int chr, string expectedKeyword)
        {
            ...
            var analysisResult = GetAnalysisWithErrorReporting(rhe, lineChr);
            var keywordToken = analysisResult.KeywordToken;
        ...
        [DataRow(0, 10, "")]
        [DataRow(1, 0, "")]
        [DataRow(1, 1, "Module TestModule")]
        [DataRow(3, 0, "Module TestModule")]
        [DataRow(3, 4, "Module TestModule")]
        [DataRow(3, 5, "Module TestModule / Entity TestModule.Pero")]
        [DataRow(6, 4, "Module TestModule / Entity TestModule.Pero")]
        [DataRow(6, 5, "Module TestModule")]
        [DataRow(9, 0, "Module TestModule / Entity TestModule.Empty")]
        [DataRow(12, 22, "Module TestModule / Entity TestModule.SameLine")]
        [DataRow(14, 0, "")]
        public void CorrectConceptContexts(int line, int chr, string expectedContext)
        {
            ...
            var analysis = GetAnalysisWithErrorReporting(rhe, lineChr);
            Console.WriteLine(analysis.TextDocument.ShowPosition(lineChr));
            var context = analysis.ConceptContext;
            var contextDesc = string.Join(" / ", context.Select(a => a.GetUserDescription()));
            Console.WriteLine($"Context at cursor ({line}, {chr}): {contextDesc}");
            Assert.AreEqual(expectedContext, contextDesc);
        }

        [DataTestMethod]
        [DataRow(2, 0, "Module M")]
        [DataRow(4, 0, "Module M / Entity M.E")]
        [DataRow(4, 5, "Module M")]
        [DataRow(5, 4, "Module M")]
...
";
            ...
            var contextDesc = string.Join(" / ", analysisResult.ConceptContext.Select(a => a.GetUserDescription()));
            Console.WriteLine($"Context at cursor ({line}, {chr}): {contextDesc}");
            Assert.AreEqual(expectedContext, contextDesc);
        }
        ...
        public void SameLineVsNewLineBraces()
        {
            ...
            })
            {
                ...
                var analysisResult = GetAnalysisWithErrorReporting(rhe, data.Item2);
...
// s
        ...
        public void ConceptActiveParameter(string script, string expectedActiveParams, string scriptPostfix)
        {
            ...
            foreach (var pair in withParam)
            {
                ...
                Console.WriteLine($"{conceptType.TypeName} ==> {ConceptTypeTools.SignatureDescription(conceptType)}");
                var members = ConceptTypeTools.GetParameters(pair.conceptType);
                var memberName = pair.activeParamater < members.Count ? members[pair.activeParamater].Name : "FULL";
                var formatted = $"{conceptType.TypeName}:{memberName}";
        ...
        public void ConceptActiveParameterEof()
        {
            ...
            var analysis = GetAnalysisWithErrorReporting(rhetosDocument, lineChr);
...
";
...

            {
                ...
                var signatureHelp = rhe.GetSignatureHelpAtPosition(lineChr);
                Console.WriteLine($"Chosen signature: {signatureHelp.signatures[0].Signature}\nActive parameter: {signatureHelp.activeParameter}\n");
                Assert.AreEqual("SimpleReferencePropertyInfo", signatureHelp.signatures[0].ConceptType.TypeName);
        ...
        public void TokenBeingTyped(string script, int line, int chr, string expectedToken)
        {
            ...
            var analysisResult = GetAnalysisWithErrorReporting(rhe, new LineChr(line, chr));
