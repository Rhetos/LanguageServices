...
namespace Rhetos.LanguageServices.Server.Tools
{
    ...
    {
        public static string SignatureDescription(ConceptType type)
        {
            ...
            var keyword = type.KeywordOrTypeName;
        ...
        {
            return $"{conceptMember.Name}: {conceptMember.ConceptType?.TypeName}";
        }

        public static List<ConceptMemberSyntax> GetParameters(ConceptType conceptInfoType)
        {
            return conceptInfoType.Members
                .Where(member => member.IsParsable)
                .ToList();
        }

        public static int IndexOfParameter(ConceptType conceptInfoType, ConceptMemberSyntax member)
        {
